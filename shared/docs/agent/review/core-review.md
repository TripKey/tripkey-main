...
Does any browser-facing flow bypass the backend?
Does the non-AI flow typically follow:
frontend -> backend -> database
Does the AI-dependent flow typically follow:
frontend -> backend -> ai-engine -> backend -> database
Does frontend call ai-engine directly?
Has any responsibility moved to the wrong layer?
Does any change imply public exposure of an internal service (ai-engine)?
